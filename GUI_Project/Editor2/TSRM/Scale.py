...
class ScaleDialog(QDialog, AdditionalDialogMethods):
    ...
    def initUI(self, figure: Figures):
        ...
        if figure == Figures.LINE:
            ...
            confirm_2D = QPushButton("Confirm"); confirm_2D.clicked.connect(lambda: self.scaleLine(self.figure, self.points, scaler_X.value(), scaler_Y.value()))
            #layout
    ...
    #Slots
    def scaleLine(self, figure: Figures, points: list[QPointF], scaleX, scaleY):
        if figure == Figures.LINE:
